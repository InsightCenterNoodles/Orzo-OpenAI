...
class EventHandler(AssistantEventHandler):
    """
    Event Handler class based on Open AI template
    handles function calls through the handle_requires_action and submit_tool_outputs method
    Subject to change as the assistants open AI api changes. This version currently supports streaming"""
    ...
    def handle_requires_action(self, data, run_id):
      """
      When a users request triggers a function call, pass it on to submit_tool_outputs. 
      """
      tool_outputs = []
      for tool in data.required_action.submit_tool_outputs.tool_calls:
        if tool.function.name == "Invoke_Method":
...
def submit_tool_outputs(self, tool_outputs, run_id):
      """
      Create new stream to submit tool_outputs.
      As of 05/10/24, this should enable successive runs but currently there must be a time delay between runs.
      Modification of this method will be needed in the future, or the API will correct this issue. 
      """
      convo = "Conversation: "
...
class Window(mglw.WindowConfig):
    ...
    Most work happens in the render function which is called e4ry frame
    """
    ...
    def key_event(self, key, action, modifiers):
        ...
        # Handle key presses like quit and toggle cameras
    ...
    def render_text_dialogue(self):
        """
        Starts ChatGPT conversation diolouge box and processes user input and the GPT response
        """
        ...
        # Input text field for User 1's notes
            ...
            if imgui.button("Send to Chat"):
                chat_dictionary = self.send_to_chat(self.user1_input)[0]
                print("here is chat_dictionary and type", chat_dictionary, type(chat_dictionary))
                self.invocation_station(chat_dictionary)
                self.conversation_log.append("User 1: " + self.user1_input)
                convo_output = chat_dictionary['Conversation_Output']
                self.conversation_log.append(chat_response + convo_output)
                self.gpt_args.append(chat_response)
                ### Clear input
                self.user1_input = ""
        imgui.end() 
    ...
    def send_to_chat(self, user_input):
        """
        Begins streaming thread and returns the response.
        Provides available methods from Document and an example prompt to give the assistant context.
        Will require modifcation as assistants API evolves.
        Parameters: user_input: string, user request to GPT model
        Returns: Dictionary containg Conversation_Output, Method_ID< color, position and name. 
        Aletring the API JSON function required items changes what is displayed, 
        Example: {'Conversation_Output': 'Invoking create aorta at specified position and color', 'Method_ID': 'create_aorta', 'color': '0,255,0', 'position': '40,20,12', 'name': None}
        """ 

        assistant_id = 'your_ assistant_key'

        document = self.client.state["document"]
        
        example_input = """
                Based on Users request, provide the Method_ID from the provided list with positions and color based on user input.
                If the user asks for an object, provide the name of the object asked for. This will always be called with create_pixel
                Please convert names of colors into their RGB components.
                Examples:

                Input: Invoke create_aorta
                OutPut: {"Conversation_Output": "Invoking create aorta", "Method_ID": "create_aorta", "color": null, "position":null, "name": null}

                Input: Create a robot
                Output: {"Conversation_Output": "Creating a robot with available data", "Method_ID": "create_pixel", "color": null, "position":null, "scale": 1, "name": "robot"}

                Available Methods:
                
                """
        for method_id in document.methods_list:
            method_string = str(self.client.get_delegate(method_id))
            example_input += method_string
        ...
        with client.beta.threads.runs.stream(
            ...
            instructions="Please use the invoke_method function",
    ...
    def invocation_station(self, response):
        """
        Invokes methods and packages parameters for method invocation on Server side
        Handles most edge cases and raises Errors as needed
        Current implementation utilizes a simple position based array, in order of [position, color, name/extra params]
        Parameters: response: dictionary from send_to_chat method
        Returns: None, invokes method on client
        """
        ### Args will be position based, increase array size if adding more parameters. 
        args = [0,0,0]
        if response["Method_ID"]:
            method_name = response["Method_ID"]

        if response['position'] is not None:
            try:
                position = ast.literal_eval(response['position'])
                if len(position) >= 3:
                    correct_pos = [(position[0], position[1], position[2], 1)]
                    args[0] = correct_pos
                else:
                    raise ValueError("Position data is incomplete.")
            except (SyntaxError, ValueError) as e:
                raise ValueError(f"Error processing position data: {e}")
        else: args[0] = [(1,1,1,1)]
            
        if response['color'] is not None:
            try:
                color = ast.literal_eval(response['color'])
                if len(color) >= 3:
                    correct_colors = [(color[0], color[1], color[2], 255)]
                    args.append(correct_colors)
                else:
                    raise ValueError("Color data is incomplete.")
            except (SyntaxError, ValueError) as e:
                raise ValueError(f"Error processing color data: {e}")
        ### Default color white
        else: args[0] = [(255,255,255,255)]
        ...
        self.client.invoke_method(self.client.get_delegate(method_name).name, args= args)
        return